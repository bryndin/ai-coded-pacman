...
function canMove(layout, position, size) {
    const gridX1 = Math.floor((position.x - size / 2) / CELL_SIZE);
    const gridX2 = Math.floor((position.x + size / 2 - 1) / CELL_SIZE);
    const gridY1 = Math.floor((position.y - size / 2) / CELL_SIZE);
    const gridY2 = Math.floor((position.y + size / 2 - 1) / CELL_SIZE);
